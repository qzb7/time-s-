...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        def sdpa(q_chunk, k_chunk, v_chunk, mask_chunk):
            try:
                ...
                # PyTorch 2.4 has SDPA but no enable_gqa keyword. Expanding
                # each K/V group to its query heads is the documented GQA
                # equivalent; newer PyTorch keeps the original native path.
                if "enable_gqa" not in str(exc):
                    raise
                if self.config.heads_per_group > 1:
                    k_chunk = k_chunk.repeat_interleave(self.config.heads_per_group, dim=-3)
                    v_chunk = v_chunk.repeat_interleave(self.config.heads_per_group, dim=-3)
                return F.scaled_dot_product_attention(
                    q_chunk, k_chunk, v_chunk,
                    attn_mask=mask_chunk, dropout_p=0.0,
                    is_causal=is_causal, scale=self.mult,
                )

        # A variate layer flattens (batch, patch) into SDPA's batch axis.
        # Large univariate batches can therefore exceed CUDA's 65535 grid
        # limit even though each attention problem has sequence length one.
        # Chunking this independent axis is mathematically identical.
        variate_batch_chunk = 16384
        if self.is_variate_layer and q.shape[0] > variate_batch_chunk:
            outputs = []
            for start in range(0, q.shape[0], variate_batch_chunk):
                end = min(start + variate_batch_chunk, q.shape[0])
                mask_chunk = attn_mask
                if attn_mask is not None and attn_mask.shape[0] == q.shape[0]:
                    mask_chunk = attn_mask[start:end]
                outputs.append(
                    sdpa(q[start:end], k[start:end], v[start:end], mask_chunk)
                )
...
def _debug_forward_nan(name: str, t: torch.Tensor) -> None:
    """受环境变量 TOTO2_DEBUG_NAN=1 控制的前向中间张量检查，用于定位 NaN 层级。"""
    if os.environ.get("TOTO2_DEBUG_NAN", "0") != "1":
        return
    bad = int(torch.isnan(t).sum().item()) + int(torch.isinf(t).sum().item())
    print(f"[TOTO2] {name}: nan+inf={bad}/{t.numel()}", flush=True)
    if bad:
        raise RuntimeError(f"[TOTO2] non-finite at {name}: {bad} of {t.numel()}")
...
class Toto2Model(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        _debug_forward_nan("scaler.scaled_series", scaled_series)
        scaled_series = scaled_series.asinh()
        _debug_forward_nan("scaler.scaled_series.asinh", scaled_series)
        x = self.patch_proj(
            torch.cat(
                [
        ...
        _debug_forward_nan("patch_proj", x)
        x = self.transformer(x, group_ids=group_ids)
        _debug_forward_nan("transformer", x)
        ...
        _debug_forward_nan("output_head.quantiles", quantiles)
    ...
    @classmethod
    def _from_pretrained(cls, *, model_id, map_location="cpu", strict=True, mmap=True, **kwargs):
        ...
        if model_dir.is_dir() and (model_dir / "config.json").exists():
            ...
            if index_file.exists():
                ...
                if not mmap:
                    # safetensors 在 CPU 下返回 mmap 视图；eager 化可避免 obsfs
                    # 并发懒加载造成的 IO 抖动与 SHR 持续上涨。
...
class Toto2GluonTSModel(nn.Module):
    ...
    def forward(
        ...
        # Keep the tensor numerically well-defined while preserving the masks:
        # invalid observations and left padding are carried as zero, but remain
        # excluded from scaling, attention, and metric accumulation downstream.
        past_target = torch.nan_to_num(past_target, nan=0.0, posinf=0.0, neginf=0.0)
        past_target = past_target.masked_fill(
            past_is_pad.bool().unsqueeze(-2), 0.0
        )
        inputs = {
            "target": past_target,
